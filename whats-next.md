...
- `pages/template-engine.html` — plans through 10-02 complete; cover caption toolbar for all 4 captions (front + spine); EB Garamond Semi-Bold wired
- `assets/Template_Scribble/template-data.js` — fully generated by csv-to-template.js including cover: block; FP3/FP4 left-V SVG paths correct
- `csv-to-template.js` — now parses cover CSV and generates cover: block; multi-slot serializer comma bug fixed
...
## Known issues
- **Cover CSV sync** — now fully working. `window.SCRIBBLE_DATA.cover` is defined in template-data.js from the CSV. Edit CSV → run `node csv-to-template.js` → cover block updates.
