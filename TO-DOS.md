...
| 45 | Order form: image quality indicator on FP photo upload | Low | Next order form pass | After customer uploads a photo for a functional page (birthday, toy, etc.), show a green checkmark or quality badge (resolution OK / LOW RES). Same async check as main pool, surfaced inline per upload zone. |
| 46 | Photo count simulation: all orientation combos | Medium | Before engineering precision | Run a simulation across all spread variant combinations (all-H, all-V, mixed) to see how much the slot count varies from the H-only estimate in calcPhotoTarget(). Determines whether we need a range instead of a single number, or can say the number is exact. `scripts/` or standalone node script. |
