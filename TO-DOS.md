...
| 38 | Spread preview: caption spec in sizing table | High | Before extending tool to full template | Kseniia's sizing table needs two columns per spread: `captions_allowed` (y/n) and `caption_position` (above/below). Tool reads these to show/hide caption UI and position overlay correctly per spread. `pages/spread-preview.html` |
| 39 | Spread preview: photo pool + orientation-enforced UX | High | After full template spec is ready | Replace drag-between-slots with: (a) photo pool at top showing all uploaded photos with H/V label and used/unused state; (b) click/× to eject photo from slot back to pool; (c) orientation enforcement — refuse H→V slot drops with red flash; (d) cross-spread moves go via pool as intermediary. `pages/spread-preview.html` |
