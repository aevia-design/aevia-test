...
| 79 | 🔴 newborn/scribble/papercut state `21 × 21 cm` — owner says all books are 20 × 20 | High | **S140, owner: "all templates are 20×20. period. we don't have landscape books."** wander was fixed in S140 (`33 × 24 cm (landscape)` → `20 × 20 cm (square)`, owner-confirmed). **Still outstanding: newborn / scribble / papercut all say `21 × 21 cm (square)`** in the `Print & production` accordion; tender + joyride correctly say `20 × 20 cm`. If the owner's "all 20×20" is literal, these 3 are a **factual error about the physical product, customer-facing and live** (same class as the "we deliver across the EU" bug fixed in S140) — left unfixed only because he explicitly confirmed wander and not these. **Confirm, then fix the `<li>Format:` line on those 3. Do it BEFORE the German build** or the wrong size gets translated into DE too. ⚠ Also re-check the other spec `<li>`s (Cover/Paper/Binding) — same placeholder block, wander's Format was wrong, so treat them all as unverified. **Lesson (S140): the specs on the product pages are NOT a source of truth — don't build copy claims on them.** |
