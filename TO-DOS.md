...
| 115 | Replace Scribble's NT Somic — it has no **ß** | Owner's call (S180): source a face that carries ß. NT Somic has ä ö ü é but no ß, and it is Scribble's **default caption font on every spread**, so a German caption or cover with "groß"/"süß" cannot be set — and a missing glyph draws nothing rather than erroring. Blocks the first German Scribble order (the customer types the cover text). Verify any replacement with `node scripts/check-font-glyphs.mjs`. Fallback: default German Scribble captions to EB Garamond, already in its picker. **Never substitute "ss"** — valid in Switzerland, wrong in Austria. |
