...
| 61 | Art gallery (FP5) loses one photo on upload | High | FP5 is the only 2-photo special page. `order.html:1370` sends `addonSlug:'fp5'` for BOTH art photos (with a `slotIndex` rider), but `functions/upload.js:103-104` names the file `${slug}.${ext}` and ignores `slotIndex` — so art-1 and art-2 both write to `special_pages/fp5.jpg` and the second overwrites the first in GCS. Render side expects two files (`template-engine.html:1267-1272`, `:3626-3630`). Fix: include slotIndex in storedName when present (e.g. `fp5-0.jpg`/`fp5-1.jpg`); redeploy `uploadPhotos`; confirm load path reads both. Found S23 (QA run on AEV-024). |
