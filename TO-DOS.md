...
| 4 | Build PDF export pipeline (Sharp + pdf-lib) | High | NEXT — Plan 11 | DECIDED 2026-05-22 (/solutioning): NOT Puppeteer — browser PDF caps photo quality at preview resolution. Standalone Node tool first (composite original photos at 300dpi, 3mm/18mm bleed = bgColor), then wire to GCS/order path (Plan 12). Specs in `LEARNINGS.md`. See `whats-next.md` Plan 11. |
...
| 41 | Build Scribble template engine — Plans 06–11 complete. Plan 12 (order flow integration) in progress. See `whats-next.md`. | High | In progress | `pages/template-engine.html` + `assets/Template_Scribble/scribble-data.js` + `scripts/export-pdf.js`. Engine, captions, PDF export (preview + print modes, cover) all done. |
...
| 42 | scribble.html: FP selector + photo count calculator | High | After template engine renders correctly | Moved from bloom.html to scribble.html. Add FP1–FP5 optional add-on cards with placeholder images + live photo count. |
| 44 | Book language option (English / German) | Medium | Before German market push | Customers should be able to order the book in English or German (printed captions, FP text labels, etc.). English only for now. Add language selector to scribble.html and pass to order. Decide: per-template or site-wide setting. |
| 47 | Mobile responsiveness — homepage + order form | High | Before any public sharing or marketing | Neither `pages/home.html` nor `pages/order.html` is properly optimised for mobile. Confirmed broken 2026-05-27. Template engine is staff-only so mobile not needed there. Priority pages: home (marketing/first impression), order form (customers submit on phone). |
| ~~45~~ | ~~Order form: image quality indicator on FP photo upload~~ | ~~Done~~ | ~~2026-05-25~~ | Async check in `handleSpecialFile()`: loads image into a hidden `Image()`, checks `min(naturalWidth, naturalHeight) < 1500`. Shows `.low-res-badge` (red "LOW RES") or `.res-ok-badge` (green "✓ OK"). JPEG/PNG only; HEIC skipped. |
| ~~46~~ | ~~Photo count simulation: all orientation combos~~ | ~~Done~~ | ~~2026-05-25~~ | `scripts/simulate-photo-count.js` written and run. Result: H and V variants always have identical slot counts in Scribble — calcPhotoTarget() is exact for all orientation combos. No code changes needed. Re-run if new spread types added. |
