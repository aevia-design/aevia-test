...
## Status
**Session 29 (2026-06-04) — Scribble E2E regression gate run; caught + fixed 2 real Scribble-blocking bugs; shipped a small UX batch. Scribble now VERIFIED working end-to-end by Evgeny** (order → engine render → save → customer preview → approve → Stripe pay → PDF). **All S29 changes UNCOMMITTED; S28's two commits (`c3279d1`, `2b993d0`) still unpushed.**

Fixed: (1) **Cover render crash** — `COVER_BLEED_MM is not defined` blanked the customer preview; S28 regression (const left out of `renderCover` scope when caption layout was extracted); Wander's empty cover slot hid it. Re-declared in the `if(slotDef)` block. (2) **False "1 empty photo slot" blocked approve** — `checkBookComplete` counted special-page (Story-page) slots' intentional `null` as empty. Added a `specialSlots` param to the shared guard; both engines now record special-slot positions during render (`window._specialSlots`) and pass them at approve/save. **83/83 tests.**

UX batch: (3) **Story-page photos locked on customer side** — non-draggable/droppable, toast on interaction (`SPECIAL_LOCK_MSG`); also closes the write-path behind bug #2. Decision logged in `docs/decisions/0004-story-page-photos-locked.md` (lock now; defer customer replace/upload). (4) **Rename customer tag "Functional" → "Story page".** (5) **PDF filenames now order-prefixed** (`${orderNumber}_preview.pdf` / `_print.pdf`); `getPdfUrl` updated to match — ⚠ **needs `firebase deploy --only functions:getPdfUrl`**. See `sessions/2026-06-04-s29.md`.

### ▶ NEXT SESSION (Session 30)
1. **Commit + deploy.** Recommended: commit S29 as one logical commit; push S28+S29 (E2E is green → safe) so the customer-facing fixes (cover crash, completeness, Story-page lock, rename) reach live; run `firebase deploy --only functions:getPdfUrl` for the PDF-naming change.
2. **Build feature #1 — original photo filenames (Option B, APPROVED).** Staff see customer's original filenames (e.g. `IMG_2156`) + sort fallback by them; customer hides names. `originalName` is captured at upload (`functions/upload.js:122`) but discarded at `upload.js:139` — persist it as display+sort metadata, keep `photo_001` as the stable internal key. Touches `upload.js`, `getOrder`, both engines' thumbnails + sort. Needs a fresh upload to test.
3. **Build feature #2 — approve→pay flow fix (researched, recommended, not yet approved).** Keep explicit approval; add Review→Approve→Pay cue; replace terminal "we'll be in touch" post-approve copy with forward "one last step: pay" copy; move "we'll be in touch" to post-payment only (`customer-preview.html:705-706`); show price on Pay button. Copy/markup only.
4. **De-hardcoding → template contract** (Evgeny's architecture ask): engine still hardcodes Scribble-shaped values (cover canvas `1227×600` in CSS, gradient fallbacks, bleed). Data file = single source of truth; migrate one value at a time, Scribble-parity-gated. See memory `project_adding_templates` + `project_bleed_model`.
5. **chunk-022 (map page)** — still blocked on Kseniia's corrected region SVGs. **Finish chunk-010** — wire Wander product page end-to-end.
