...
## Status
**Session 28 (2026-06-04) — chunk-010 (Wander) advanced: Wander now renders end-to-end in the staff engine with local photos.** Two commits (`c3279d1`, `2b993d0`), **NOT pushed**. Done this session: (1) **Registered Cormorant Garamond** on all 3 surfaces (engine + customer `@font-face`, export-pdf `FONT_FILE_MAP`, cover font picker). (2) **Fixed a renderCover crash** — `coverDef.slots[0].wMm` threw on Wander's free-text cover (`cover.slots: []`), blanking the whole book; guarded on `slotDef`, mirrored in customer-preview. (3) **Local-mode template selector** (`#template-select`, Scribble/Wander) — FP checkboxes + cover-photo zone now data-driven from the active template; also fixed a latent order-load bug (didn't rebuild FP UI for the order's template). (4) **Temp FP1 map stub** (chunk-022 still blocked on Kseniia's region SVGs). (5) **Wander cover bleed bug RESOLVED** — its cover SVG viewBox was full-bleed instead of content-framed; reframed to `212.598 212.598 4818.9 2362.2`; the whole **bleed model is now documented** in memory `project_bleed_model.md`. (6) **Sequencing log clarity** — verbose per-spread dump → one-line summary + a small amber ⚠ badge on out-of-order photos in the book (staff-only). (7) Wander spread SVGs re-exported (graphic fixes). **Verified:** render smoke (cover + 41 page-canvases, 0 errors), 80/80 tests, font load, selector behaviour. New QA scripts: `qa/verify-wander-fonts.mjs`, `qa/check-selector.mjs`, `qa/debug-wander-render.mjs`. **⚠ Scribble E2E NOT re-run — required before deploy** (this session changed shared render paths). See `sessions/2026-06-04-s28.md`.

### ▶ NEXT SESSION (Session 29)
1. **Scribble E2E before any push/deploy** — run `qa/staff-customer-chain.mjs` on a Scribble order (order→save→customer→Stripe→PDF, last verified S24). Then `git push` the two S28 commits.
2. **De-hardcoding → template contract** (Evgeny's architecture ask): engine still hardcodes Scribble-shaped values (cover canvas `1227×600` in CSS, gradient fallbacks, bleed assumptions). Direction agreed: data file = single source of truth; CSV stays authoring input (no runtime parse); migrate one value at a time, Scribble-parity-gated. Start with cover geometry + write the contract doc. See memory `project_adding_templates` + `project_bleed_model`.
3. **chunk-022 (map page)** — still blocked on Kseniia's corrected region SVGs.
4. **Finish chunk-010** — wire Wander product page end-to-end (`wander.html` already sends `template:'Wander'`); confirm a real Wander order renders + PDFs.
