# Session Status
_Last updated: 2026-07-10 (session 118)_
_Context at save: Shipped referral first-order enforcement (committed, NOT deployed) + two Safari layout fixes (instruction-bar centring + full responsive scale-to-fit for the book preview), all pushed to main. Owner deferred the referral deploy and will run it + a real-Safari check next. Enabled the missing rageatc-code-oss plugin (needs a Claude Code restart)._

## Status
**Session 118 (2026-07-10) — REFERRAL first-order enforcement + SAFARI responsive fixes. THREE commits pushed to `main`. (1) REFERRAL (`c77c528`, NOT deployed): the share code now truly applies only on a customer's FIRST paid order, and Stripe + our order DB stay consistent. Root cause: `createCheckoutSession` attached NO Stripe Customer, so the code's `first_time_transaction` restriction never bound and a returning buyer could reuse the €10. Fix = find-or-create a Stripe Customer keyed by normalized email on EVERY checkout (cached at `customers/{email}.stripeCustomerId`) → Stripe now rejects the code on a 2nd+ paid order. Plus a DB-side guard: new `countPriorPaidOrders` + a `refereePriorPaidOrders` rule in `referrerRewardDecision` (reason `referee_not_first`) so no reward issues for a non-first referee. 3 new tests → 186/186. (2) INSTRUCTION BAR (`a643eb1`): centred the customer-preview `.edit-hint` → two balanced lines. Measured it's ~1580px and overflows any laptop in EVERY browser (WebKit/Chromium within 1px) — a width issue, NOT a Safari bug. (3) RESPONSIVE (`e9484c1`, customer-preview only): interior spreads now scale-to-fit like the cover (`fitCover`→`fitBook`, new `.spread-scale-wrap`, removed the viewport-keyed stacking query) so the book fits any window; the instruction bar reserves its height live (`adjustHintReserve`) so it never overlaps the first header; and the ✥ reposition drag now divides the pointer delta by the on-screen scale (`attachCropDrag`) — fixed a real scaled-spread bug AND a latent cover bug. `/critic-agent` caught the drag bug (the plan's "the cover already proves drag works under scale" was FALSE — same code, just barely scaled). Verified in WebKit + Chromium at 6 widths + a quantitative drag-gain test. Also enabled the missing `rageatc-code-oss` plugin in `~/.claude/settings.json` (only core-oss was on — that's why `systematic-debugging` etc. were absent; needs a Claude Code restart). Full log: `sessions/2026-07-10-s118.md`.**

- **▶ RESUME HERE (S119):** Two owner actions pending. (a) **Verify the Safari responsive fix** on a real Mac — open a test-order preview link (hard-refresh), Edit mode at 100%: the whole book (cover + spreads) fits the window, the instruction bar clears the first spread header, and the ✥ reposition tracks the cursor 1:1. (b) **Deploy + sandbox the referral fix** (owner deferred): `npm install -g firebase-tools` → `firebase login --no-localhost` → `firebase deploy --only functions:createCheckoutSession,functions:stripeWebhook` (CLI is NOT installed in a fresh Codespace); then Stripe Test-mode — normal order pays, FRIENDS30 discounts, first referral gives −€10, and the SAME email on a 2nd order gets the code REJECTED. Standing candidates unchanged: print-mode PDF / print-house API; render-pipeline extraction; SEPA/EPS; client analytics (parked).
- **Watch-out (S118):** REFERRAL is committed but UNDEPLOYED — `createCheckoutSession` + `stripeWebhook` BOTH changed and both must be in the deploy; the change touches EVERY checkout, so regression-test a plain no-code order too. A customer who ordered as a GUEST before this deploy has no Stripe Customer yet → their next order still looks "first" to Stripe (bounded €10; the DB guard backs it up). RESPONSIVE is customer-preview ONLY — the staff engine shares the fixed 1227/600px sizing and still overflows (deferred; one internal user). Spreads scale between ~900–1544px; below 900 the existing mobile gate takes over. The `.edit-hint` bar exists ONLY in customer-preview (NOT the staff engine — an earlier plan assumption was wrong). `.claude/settings.local.json` still modified (pre-existing, uncommitted). `rageatc-code-oss` is enabled but needs a Claude Code RESTART to load. Full responsive plan + critic fixes: `~/.claude/plans/i-see-i-mean-velvety-mango.md`. Codespace is DISPOSABLE — push everything; firebase-tools + `.env` need re-adding in a fresh one.

### Previous: Session 117
**Session 117 (2026-07-10) — CODESPACE NOW FULLY WORKING + a small Tender fix shipped. (1) Confirmed the S116 personal-setup migration is COMPLETE: `~/.claude/` has skills + agents + global CLAUDE.md + this project's auto-memory (recall works), rageatc plugin skills are back (`/handover`, `/prime`, `/checkpoint`), `npm test` = 183/183, no leftover bundle folder in the tree. (2) Walked the owner through the Codespace working model (new to him): template folders still live at `assets/Template_<Name>/`; git is the sync mechanism (GitHub = permanent hub, one `git pull` catches the laptop up when it's back from repair, only secrets don't travel via git); the Codespace is disposable so nothing is safe until pushed; add files by drag-drop into VS Code; open CSVs tabular via Rainbow CSV or Edit CSV (set separator to `;`); reach the local dev server via the Ports-tab forwarded URL, not `localhost`. (3) TENDER OVERLAY FIX — commit `f515828` pushed to `main` → Cloudflare: decorative art now renders BELOW the photo on Spread 0 right (H/V) and Spread 3 right (H/V). This is a data flag, not a code bug — CSV `overlay_position=below` → `overlayBelow:true` in `tender-data.js`, read by all three renderers (staff engine, customer preview, PDF). Edited CSV (canonical) + data.js together (no auto-generator). Session flip-flopped once (owner first said "above", then corrected to "below" and edited the CSV himself). 183/183 tests; owner verified locally over the forwarded port before push. Full log: `sessions/2026-07-10-s117.md`.**

- **▶ RESUME HERE (S118):** No forced next step. Top carried item: **owner runs the referral sandbox test** (carried S115→S116→S117) — full flow incl. redeeming the same share code TWICE with the same referee email (`first_time_transaction` guest gap); the "Refer a friend" account tab is LIVE but untested. If Stripe doesn't block the second redemption and the owner wants it closed → option (b) Stripe Customer attach in `createCheckoutSession` (needs owner OK). Standing candidates unchanged: print-mode PDF / print-house API; render-pipeline extraction into a shared module; SEPA/EPS; client analytics (parked).
- **Watch-out (S117):** `.claude/settings.local.json` is modified in the working tree (pre-existing, unrelated) — left uncommitted deliberately. Tender overlay: CSV `overlay_position` ↔ `overlayBelow` in `tender-data.js` must always be edited together (no generator); Spread 3's LEFT pages keep art-below, right pages now match. Codespace is DISPOSABLE (~30 days idle → auto-delete) — push everything; only `.env` files need re-pasting into a fresh one. Firebase deploys from the Codespace need `firebase login --no-localhost`.

### Previous: Session 116
**Session 116 (2026-07-08) — LAPTOP→CODESPACE MIGRATION (side topic, no product code). (1) Committed + pushed everything that lived only on the laptop: `9e19b33` = the whole S115 referral build (the S115 "NOTHING COMMITTED" warning is RESOLVED — but the sandbox test is still owed, and the push put the "Refer a friend" account tab LIVE on Cloudflare); `2071bea` = 21 session logs s90–s111, 5 briefs, `context/tone-of-voice.md`, `assets/email set-up/`. 183/183 tests before committing. Secrets (`functions/.env`, `serviceAccountKey.json`, `motif-engine/.env`) copied to owner's cloud. (2) Codespace `miniature-xylophone` created on main: `npm test` 183/183 inside it, `.env` pasted in, `claude` CLI installed + logged in (terminal Claude WORKS). Browser-Chrome Claude PANEL broken there (webview service-worker CSP error; cookie exceptions didn't fix) → decision: owner connects with DESKTOP VS Code + "GitHub Codespaces" extension from a Mac (first Mac too old for VS Code; second Mac OK). (3) Personal-setup bundle `claude-setup-bundle.zip` (0.5 MB, OneDrive Desktop) = personal `~/.claude` skills + global CLAUDE.md + agents + this project's auto-memory + the repo's GITIGNORED `.claude/` extras (add-template skill, hooks, settings.json). Owner uploaded it into the codespace but Mac Safari auto-unzipped it; folder location unclear — INSTALL COMMANDS NEVER RAN. Full log + exact finish commands: `sessions/2026-07-08-s116.md`.**

- **▶ RESUME HERE (S117 — probably running INSIDE the codespace):** (1) **Finish the Claude setup migration**: locate the unzipped `claude-setup-bundle` folder (likely somewhere under `/workspaces/aevia-test`), then copy `claude-home/.` → `~/.claude/`, `project-memory` → `~/.claude/projects/<this-project's-slug>/memory` (verify the slug the codespace actually uses), `project-claude/.` → repo `.claude/`; DELETE the bundle folder from the working tree (never commit it); restart Claude and verify `/checkpoint`+`/prime` appear and project memory recalls. Exact commands: `sessions/2026-07-08-s116.md`. (2) Carried S115 item: **owner runs the referral sandbox test** — full flow incl. redeeming the same share code TWICE with the same referee email (`first_time_transaction` guest gap); if Stripe doesn't block and owner wants it closed → option (b) Stripe Customer attach in `createCheckoutSession` (needs owner OK). (3) Standing candidates unchanged: print-mode PDF / print-house API; render-pipeline extraction; SEPA/EPS; client analytics (parked).
- **Watch-out (S116):** The codespace is DISPOSABLE (~30 days idle → auto-delete) — commit+push everything worth keeping; only the `.env` files need re-pasting into a fresh one. Referral account tab is LIVE while sandbox-UNTESTED. rageatc plugin skills (`/handover`, `/solutioning`, `/shaping`) are NOT in the bundle — reinstall the plugin in the codespace or write handovers manually (session file + STATUS.md + name the completed session). Firebase deploys from the codespace need `firebase login --no-localhost`. Laptop's leftover untracked files are throwaway only — safe to lose.

### Previous: Session 115
**Session 115 (2026-07-07) — REFERRAL PROGRAM (promo codes Phase 2) BUILT + REVIEWED, UNCOMMITTED. Built per `docs/briefs/promo-codes.md`: (1) `functions/referral-utils.js` (NEW, pure/testable) — `ANNA-7K2P` code generation (no I/L/O/0/1), promo-code extraction from a Stripe session, and the reward decision (paid + known code + not already rewarded + not self-referral). (2) `tests/referral.test.js` (NEW) — 16 tests, TDD; suite now 183/183. (3) `functions/index.js` — NEW `getMyReferralCode` Cloud Fn (verified-token gate; mints a multi-use share code with `first_time_transaction`, backed by env `STRIPE_REFERRAL_COUPON_ID`; writes the `referralCodes/{promotionCodeId}` reverse index + mirrors onto `customers/{email}`; transactional with orphan-code cleanup) + `handleReferralReward` in `stripeWebhook` (isolated try/catch — can never break the paid flow; mints a single-use €10 `THANKS-XXXX` reward, 12-month expiry; order `referredBy`+`referrerRewardIssued` and customer `rewardCodes` written in ONE atomic batch; emails the referrer). (4) `pages/account.html` — "Refer a friend" tab (share code + Copy, reward codes list). `/reviewer-agent` verdict REVISE → both required fixes applied (mint race → transaction; reward writes → batch). Copy stop-slopped. Full log: `sessions/2026-07-07-s115.md`.**

- **▶ RESUME HERE (S116):** (1) **Owner reports the sandbox test** — full flow: account → Refer a friend → code → second-email test order redeems it (−€10) → pay `4242…` → referrer gets the `THANKS-` email + code appears in their account. **Crucially: try redeeming the same share code TWICE with the same referee email** — this answers the review's key open finding (below). (2) If Stripe does NOT block the second redemption and the owner wants it closed → build option (b): look up/attach a Stripe Customer by order email in `createCheckoutSession` (touches ALL checkouts — needs explicit owner OK). (3) **COMMIT the S115 work** (referral-utils.js, referral.test.js, index.js, account.html, session log) once the sandbox test passes — it is all working-tree only. (4) Standing candidates unchanged: print-mode PDF / print-house API; render-pipeline extraction; SEPA/EPS; client analytics (parked).
- **Watch-out (S115):** **NOTHING from S115 is committed** — don't discard the working tree. `getMyReferralCode` is a NEW Cloud Fn → include in every future `firebase deploy --only functions` (new fns to date: `reportOrderIssue`, `sendPreviewEmail`, `sendPasswordChangedEmail`, now `getMyReferralCode`; plus `stripeWebhook` changed). Referral needs `STRIPE_REFERRAL_COUPON_ID` in `functions/.env` (owner added the Test-mode id; Live needs the coupon re-created there + a new id, same as FRIENDS30). **Key open finding: `first_time_transaction` may not bind for guest checkouts** (no Stripe Customer is created/reused) — the same friend could redeem a share code on a second order; damage bounded at €10/order, self-referral blocked, reward codes strictly single-use. Coupon-level date/redemption caps deliberately left EMPTY (they'd apply globally and kill the whole programme). 4-char suffix fine at F&F scale; lengthen before public scale-up (comment in referral-utils.js).

### Previous: Session 114
**Session 114 (2026-07-07) — STAFF DASHBOARD (via `/shaping` + `/solutioning`). Reviewed order statuses, built + shipped a cleanup + a customer→staff issue channel. ONE commit `38f3c01` pushed to `main` → Cloudflare. (1) DROPPED `designing` + `needs_info` — both were dead statuses (badge/filter/count but NO workflow behind them; owner confirmed nothing fires on `needs_info`). Orders now sit at `new` from upload until a preview is sent. Decision: `designing` would only earn its place if auto-set (e.g. on first book-save), worth it only with >1 designer — owner is solo (Xenia), so collapsed to `new`. (2) NEW `issue` side-state + report channel: a "Report an issue" link+modal in `customer-preview.html` → new `reportOrderIssue` Cloud Fn (token-gated) that saves the note, flips a `review_sent` order to `issue`, and emails `support@aevia.at` (order# + message). Dashboard: red-tinted row + inline ⚠ note, an **Issues** filter + stat card, issue orders float to the top (stable sort). Customer sees status "Under review". (3) Customer label fix in `account-utils.js`: `paid → "Payment confirmed"` (was "Approved", which undersold a paid order); `issue → "Under review"`. Files: `functions/account-utils.js`, `functions/index.js` (new fn), `pages/staff/dashboard.html`, `pages/customer-preview.html`, `tests/account-utils.test.js`. Copy stop-slopped. `npm test` = 167/167. Concept doc `work/dashboard-statuses/concept_v1.md`. Then `/solutioning`: considered surfacing per-order payment/promo detail on the dashboard → DECIDED AGAINST (Stripe is the payment source of truth; referral attribution is webhook-automated in the Phase 2 spec, so the dashboard doesn't need the code; YAGNI at test-order volume). Also corrected an earlier mis-claim: dashboard quick-find (order#/name/email search) ALREADY exists. Full log: `sessions/2026-07-07-s114.md`.**

- **▶ RESUME HERE (S115):** No forced next step. Prime candidate is still **Phase 2 (referral)**, specced in `docs/briefs/promo-codes.md` — built as-is, with NO dashboard payment-detail addition (decided against S114). Standing candidates unchanged: print-mode PDF / print-house API (everything past `paid` is unbuilt — the print-hub track); render-pipeline extraction into a shared module (engine-parity tripwire); SEPA/EPS bank payment methods (DACH fee lever, Stripe-config); client analytics (still PARKED — needs a concrete "if I saw X I'd change Y" before it's worth building; GDPR + cost to weigh).
- **Watch-out (S114):** `reportOrderIssue` is a NEW Cloud Fn — must be in every future `firebase deploy --only functions`, and the customer labels ("Payment confirmed"/"Under review") only appear once functions are deployed (`getMyOrders` reads `account-utils.js` server-side). The **Report-an-issue link stays visible after approval/paid**, where it emails `support@` but does NOT flag the dashboard or change status (deliberate — never knock a paid order back); only `review_sent` orders flip to `issue` + surface on the dashboard. `issueNote` persists in Firestore after resolution (hidden once status leaves `issue`; a re-report overwrites). Any legacy orders still in `designing`/`needs_info` should be bumped to `new`/`review_sent` (the staff dropdown no longer lists those two). Owner is running the functions deploy now; verify on live (hard-refresh) with a `review_sent` test order → Report an issue → support@ mail + dashboard flag + account "Under review" → resend preview drops it back to `review_sent`.

### Previous: Session 113
**Session 113 (2026-07-07) — PROMO CODES: shaped + brief'd + critic-reviewed a full promo-code system, then built and live-verified Phase 1 (F&F code) in Stripe sandbox. Two commits pushed to `main`. (1) `8a80824` — `docs/briefs/promo-codes.md`, the plan-of-record. (2) `74168a5` — one line in `functions/index.js` `createCheckoutSession`: `allow_promotion_codes: true`, which shows Stripe's hosted "Add promotion code" field (inert until a coupon exists). Decision: use STRIPE NATIVE Coupons/Promotion Codes, not a custom store — Stripe enforces every abuse mode (expiry, max_redemptions, per-customer caps, first-order-only, no stacking) server-side. Phase 1 = shared `FRIENDS30`, 30% off, capped by BOTH 50 redemptions AND expiry 2026-08-31, book-product-only; owner deployed the fn, created the Test-mode coupon + code, and ran the full flow (order → enter code → 30% off → test card `4242…` → order paid) — CONFIRMED WORKING. Going live = re-create the coupon in Live mode + existing key swap; no code change. Phase 2 (referral) fully specced but PARKED: per-referral €10 codes (referrer needs verified account, guest referees redeem; NOT an accumulating wallet — one €10 code per referral, Stripe allows one code per checkout), attribution via a new `referralCodes/{promotionCodeId}` index, idempotent webhook reward. critic-agent verdict REVISE → all 3 high-priority gaps closed in the brief. `npm test` = 167/167. Full log: `sessions/2026-07-07-s113.md`.**

- **▶ RESUME HERE (S114):** No forced next step. Prime candidate = **build Phase 2 (referral)**, fully specced in `docs/briefs/promo-codes.md` — new Cloud Function to mint a unique €10 promo code per verified customer (stored on `customers/{email}` + a `referralCodes/{id}` index), surface it in `account.html` (copy needs /stop-slop), extend `stripeWebhook` to attribute a referred PAID order → mint the referrer's €10 code exactly once (idempotent via `referrerRewardIssued`), unit tests in `tests/referral.test.js`. Deserves its own focused run (new fn + webhook change + UI + tests); backend deploys owner-triggered. Other standing candidates unchanged: print-mode PDF / print-house API (parked on printer's RGB→CMYK answers); render-pipeline extraction into a shared module (engine-parity tripwire); SEPA/EPS bank payment methods (cheaper-fee lever in DACH, Stripe-config not a new provider — side question raised S113, not written up).
- **Watch-out (S113):** Test-mode Stripe coupons do NOT copy to Live — `FRIENDS30` must be re-created in Live mode before real F&F orders. Phase 2 webhook must be idempotent (Stripe redelivers events). `functions/.env` holds Test-mode keys (confirmed) — going live is a key swap.

### Previous: Session 112
**Session 112 (2026-07-07) — PDF GENERATION: two bugs found + fixed + deployed + confirmed working. Two commits pushed to `main`. (1) `9bf4de4` — `generatePdf` now guards against generating a PDF for an unsaved book (empty `staffBookSequence`) with a clear "Save book state first" message instead of a silent 0%-forever hang that produced a cover-only PDF (was AEV-050's symptom; root cause = frontend pushed but the function guard wasn't deployed). Same commit: dashboard progress now shows `Rendering… N%` INSIDE the button (dropped the stale left-hand "Starting…" label); fixed the misleading engine "Export book state (JSON) → (PDF ready)" message (that JSON is a backup — the renderer reads the `staffBook*` fields written by "Save book state", NOT `book-state.json`). (2) `75b423a` — `services/pdf-renderer/index.js` `fetchAllPhotos` rewritten from an unbounded `Promise.all` (which stalled AEV-051 at 0% until Cloud Run's 900s kill on ~2GB of large pro-camera originals) to a 6-worker bounded-concurrency pool + a 120s per-photo download timeout that fails fast with the offending path instead of hanging. Owner deployed BOTH (`firebase deploy --only functions:generatePdf` + `gcloud run deploy aevia-pdf-renderer --source … --memory 8Gi --cpu 4 --timeout 900`); AEV-051 preview PDF then generated successfully. `npm test` = 167/167. Full log: `sessions/2026-07-07-s112.md`.**

- **▶ RESUME HERE (S113):** No forced next step. The S110 live-verification track is now effectively closed. Standing candidates: (a) **print-mode PDF / print-house API** — parked pending the printer's answers on file format + RGB→CMYK colour (see S112 log; `--mode print` building blocks already exist in `scripts/export-pdf.js`, not wired to the dashboard); (b) render-pipeline extraction into a shared module (the big permanent fix behind the engine-parity tripwire); (c) business-planning follow-ups from S111 (DACH-specific CAC, repurchase validation, real-buyer WTP test) if the owner returns to that thread.
- **Watch-out (S112):** The engine still has TWO similarly-named buttons — "Save book state" (writes `staffBook*`, feeds the PDF) vs "Export book state (JSON)" (writes `book-state.json`, backup only). Confusable; message fixed but buttons coexist. The renderer holds all fetched originals in memory during render — 8Gi handled AEV-051's ~2GB; if a future order OOMs at ~95% (not 0%), the clean next lever is "only fetch photos actually placed in the book" (skip unused pool). Renderer deploys from the WORKING TREE via `gcloud run deploy --source`, NOT from git push — redeploy it after any `services/pdf-renderer/` or `scripts/export-pdf.js` change.

### Previous: Session 111
**Session 111 (2026-07-07) — BUSINESS PLANNING, no code, nothing committed/deployed. Owner took an off-track session on the financial model. Shaped two coupled questions via `/understanding-the-ask` + `/shaping` + two `/deep-research` runs. (1) PRICING → working assumption €89–99, leaning €95 shipping-included (up from the €66–69 placeholder): Aevia is a done-for-you gift/occasion object, not "premium CEWE"; variable cost ≈€15–16/book = 77–84% margin so pricing is a demand/positioning question not cost-plus; the old friends' pushback test was flawed (allowed direct CEWE comparison). (2) MARKETING → kill the flat 10%-of-revenue line, model bottom-up from CAC: paid=`budget÷CAC`, organic=ramping capped assumption, referral=% of orders. Paid-only premium photobook ≈1.6:1 LTV:CAC (avoid); organic-led ≈3:1+; Artifact Uprising (closest analog) grew organic not paid → make-or-break = build the organic engine to keep blended CAC ~€30. CAC band €45–70 is US-skewed NOT Austrian. (3) CONTENT OPS → build-it-yourself with Xenia + AI (no agency yr0–1), AI polishes REAL assets never fabricates scenes; starter stack ~€30–70/mo; first-months cadence M0 setup→M3 small paid boost to MEASURE real CAC. Deliverable: `docs/briefs/business-model-pricing-marketing.md`. Full log: `sessions/2026-07-07-s111.md`.**

- **▶ RESUME HERE (S112):** No forced next step; owner returning to CODING. The S110 live-verification track is still open + unchanged: (a) verify email deploy on a fresh order (new footer-logo confirmation design); (b) live E2E the preview-send flow (generate → QA → Send preview → email + status → "View your book"); (c) eyeball S110 on live (instruction-row wrap + Preview view-only). Business-planning follow-ups (only if owner returns to it): pull DACH-specific CAC; validate ~20% repurchase; real-buyer WTP test (€90 vs €99); AOV levers; delivery fee. Carried: print-house RGB→CMYK reply, render-pipeline extraction.
- **Watch-out (S111):** all pricing/CAC numbers are WORKING ASSUMPTIONS, not decisions; the CAC band is conservative/US-skewed, not Austrian truth. No code changed this session.
