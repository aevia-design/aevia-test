...
| `verify-caption-parity.mjs` | **Run after any caption, font or engine-layout change.** Proves the S159 guarantee: the line breaks the engine RECORDS (`collectCaptionLines`) reconstruct each caption exactly and match the number of line boxes the browser actually drew — so the PDF, which now draws those lines verbatim instead of re-wrapping, cannot diverge from the approved screen. Takes a template value as argv (default `Heirloom-Beige`); passes on all 7. Caught a rotated-spine bug (one recorded line per CHARACTER) that code review missed. | no | template-agnostic |
...
| `debug-monogram-grid.mjs` | **(S195)** Screenshots every Heirloom colourway × monogram × letter pair (AM, WI, JL, MW, QG) on the back cover and intro, in the local engine (no login, no Firebase) → `sessions/qa-runs/mono/`. Needs `npm run dev`. | no | **Heirloom** |
